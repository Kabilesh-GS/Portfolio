  ...
  return (
    <RealmPage kicker="The Citadel" title="Chronicles & Writings" embers={16} >
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
        {BLOGS.map((b) => (
          <a key={b.id} href={b.url} target="_blank" rel="noreferrer" className="border border-l-8 rounded-[0.5rem] border-gold/40 lift group flex flex-col overflow-hidden bg-cover bg-center" style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.67), rgba(0,0,0,0.9)), url(${BG})` }}>
            <div className="flex flex-1 flex-col p-5">
              ...
              <span className="flex mt-3 w-max border rounded-md border-gold/40 bg-ink/80 px-2.5 py-0.5 text-[0.5rem] uppercase tracking-widest text-gold">
                {b.tag}
              </span>
